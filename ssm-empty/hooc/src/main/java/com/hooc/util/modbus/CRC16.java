...
public class CRC16 {
	...
	/**
	 * 加密数据
	 * @param puchMsg 需要加密的byte
	 * @param usDataLen 需要加密的 byte的长度
	 */
	 ...
	 * @return
	 */
	public static byte[] update(byte[] puchMsg){
		if(puchMsg==null||puchMsg.length<3){
			return null;
		}
		CRC16 crc16 = new CRC16();
		crc16.update(puchMsg, puchMsg.length-2);
		int ri = crc16.getValue();
		puchMsg[puchMsg.length-1]=(byte) (0xff & ri);
		puchMsg[puchMsg.length-2]=(byte) ((0xff00 & ri) >> 8);
		return puchMsg;
	}
	...
	public static byte[] HexString2BufNoAdd(String src) {
		int len = src.length();
	 ...
	 * @return
	 */
	public static boolean electricityCheck(byte[] b){
		if(b[b.length-1]!=0x16){
			return false;
		}
		int sum = 0;
	 ...
	 * @return
	 */
	public static byte[] electricityUpdate(byte[] bb){
		int sum = 0;
		for(byte b : bb){
			sum+=b;
		}
		return ArrayUtils.addAll(bb, (byte)(sum%256),(byte)0x16);
	}
...
//		String a = "1537AF00353030303030303030353833930AAD17";
//		byte[] b = {0x15,0x37,(byte) (0xAF),0x00,0x35,0x30,0x30,0x30,0x30,0x30,0x30,0x30,0x30,0x35,0x38,0x33,(byte) 0x93,0x0A,(byte) 0xAD,0x17,(byte) 0xE6,(byte) 0xF1};
//		System.out.println(a.length());
//		System.out.println(checkBuf(b));
//		System.out.println(CRC16.getBufHexStr(new byte[]{1,2}));
		String a = "00004843";
//		System.out.println(checkBuf(CRC16.HexString2BufNoAdd(a)));
		byte[] xx = CRC16.HexString2BufNoAdd(a);
//		System.out.println(xx[3]);
//		byte[] d = Arrays.copyOfRange(xx, 7,11);
		float float1 = ByteUtil.byteToFloat(xx);
